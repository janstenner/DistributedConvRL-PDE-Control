...
# sensor positions - 
sensor_positions = collect(1:4:nx)
actuator_positions = collect(1:4:nx)
#actuators_to_sensors = [1,4,7,10]
actuators_to_sensors = collect(1:25)
...
gaussians = prepare_gaussians(2.0)
gaussians_actuators = prepare_gaussians(2.0)
...
function reward_function(env)
    ...
    for i in 1:length(actuator_positions)
        sensors[i] = dot(y[2,:].^2, gaussians[actuators_to_sensors[i]]) / 4
    end
...
function prepare_action(action0 = nothing, t0 = nothing; env = nothing) 
    ...
    for i in 1:length(actuator_positions)
        p = p .+ action[1,i] * gaussians_actuators[actuators_to_sensors[i]]
    end
