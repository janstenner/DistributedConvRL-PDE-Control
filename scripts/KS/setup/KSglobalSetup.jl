...
y0 = FileIO.load(dirpath * "/y0.jld2","y0")
...
function prepare_gaussians(sigma = 0.8; norm_mode = 1)
    ...
    norm_mode == 2 && (positions = actuator_positions)
...
function reward_function(env; test = nothing)
    if isnothing(test)
        y = env.y .* 6
        action = env.action[:]
        delta_action = env.delta_action[:]
    else
        y = test["y"]
        action = test["action"]
        delta_action = test["delta_action"]
    end
    ...
    return [mean(sensor_rewards - action_punish * action.^2 - delta_action_punish * delta_action.^2)]
...
function initialize_setup(nna_scale = nna_scale, nna_scale_critic = nothing, drop_middle_layer = drop_middle_layer, drop_middle_layer_critic = nothing, γ = γ, p = p, batch_size = batch_size,  update_freq = update_freq, trajectory_length = trajectory_length, learning_rate = learning_rate, act_noise = act_noise, fun = relu, fun_critic = nothing; use_random_init = false)
...
initialize_setup(nna_scale, nna_scale_critic, drop_middle_layer, drop_middle_layer_critic, y, p, batch_size, update_freq, trajectory_length, learning_rate, act_noise; use_random_init=true)
...
#plot_heat(p_te = 200.0, p_t_action = 100.0, plot_separate = true, from = 90, to = 115, use_random_init = false)
#plot_heat(plot_best = true)



function test_setup(nna_scale=3.0, nna_scale_critic=3.0, drop_middle_layer=false, drop_middle_layer_critic=false, γ=0.99, p=0.995, batch_size=32, update_freq=10, trajectory_length=7_000_000, learning_rate=0.001, act_noise=0.1, fun=relu, fun_critic=relu)

    global rng = StableRNG(seed)
    Random.seed!(seed)

    #println((nna_scale, Bool(drop_middle_layer), γ, p, batch_size, update_freq, trajectory_length, learning_rate, act_noise))
    initialize_setup(nna_scale, nna_scale_critic, Bool(drop_middle_layer), Bool(drop_middle_layer_critic), γ, p, Int(batch_size), Int(update_freq), Int(floor(trajectory_length)), learning_rate, act_noise, fun, fun_critic; use_random_init=true)

    #initialize rngs again because so we always have the same initial values
    global rng = StableRNG(seed)
    Random.seed!(seed)

    nepisodes = 100

    run(agent, env, StopAfterEpisode(nepisodes), hook)

    cost = - mean(hook.rewards[end-Int(floor(nepisodes * 0.5)):end]) - sum(map(x -> (x > -0.1) * (x - (-0.1)), hook.rewards[end-Int(floor(nepisodes * 0.5)):end]))
